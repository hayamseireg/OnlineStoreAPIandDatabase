...
const update = async (req: Request, res: Response) => {
  try {
    ...
    const product: Product = await ProductStoreInstance.update(id, {
      name,
      price
    });
...
export default function productRoutes(app: Application) {
  ...
  app.post('/products/create', isAuthorized, create);
  app.get('/products/:id', read);
  app.put('/products/:id', isAuthorized, update);
  app.delete('/products/:id', isAuthorized, deleteProduct);
}
