...
describe('User Model', () => {
  ...
  it('update method should update the user', async () => {
    ...
    const newUserData: BaseUser = {
      firstname: 'hayamUpdated',
      lastname: 'seiregUpdated',
    };

    const { firstname, lastname } = await UserStoreInstance.update(
      createdUser.id,
      newUserData
    );
  ...
  it('authenticates the user with a password', async () => {
    ...
    const userFromDb = await UserStoreInstance.authenticate(
      user.username,
      user.password
    );
